...
def plot_history(model):
    ...
    #plt.plot(model.history.history["val_loss"], "k^-", label = "Val Loss")
    #plt.set_xlabel("Epoch")
    #axes[0].set_ylabel("Loss")
    #axes[0].legend()
    
    
    #axes[1].plot(model.history.history["accuracy"], "b.-", label = "Training Acc")
    plt.xlabel('epoches')
    plt.ylabel('val_accuracy')
    plt.plot(model.history.history["val_accuracy"], "k^-", label = "Val Acc")
